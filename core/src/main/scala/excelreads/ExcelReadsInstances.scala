...
trait ExcelReadsInstances {
  ...
  ): ExcelReads[R, A] = ExcelReads.from { implicit m =>
    for {
      s <- get
      aOpt <- f(sym)(s)
      _ <- put(s + 1)
  ...
  ): ExcelReads[R, Option[Style]] = ExcelReads.from { implicit m =>
    for {
      s <- get
  ...
  ): ExcelReads[R, List[A]] = ExcelReads.from { implicit m =>
    def loop(
  ...
  implicit def hConsInstances[R, H, T <: HList](implicit
    head: ExcelReads[R, H],
    tail: ExcelReads[R, T]
  ): ExcelReads[R, H :: T] = ExcelReads.from { implicit m =>
    for {
      hv <- head.parse
      tv <- tail.parse
    } yield
